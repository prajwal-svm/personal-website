...
const ProjectsData = [
    {
        ...
        githubURI: 'https://github.com/Prajwal-S-Venkatesh/prajwalsv.dev',
...
const Work = ({ id, sectionIndex, sectionTitle }) => {
    ...
    return <StackWithTitleWrapper id={id} sectionIndex={sectionIndex} sectionTitle={sectionTitle}>
        <Grid w='100%' templateColumns='repeat(1, 1fr)' gap={6}>
            {
                ...
                >
                    ...
                    <Box w='100%' h='100%' p='10' position='absolute' zIndex='2'>
                        {project.isFeatured && <Text color='primary' fontSize='md'>Featured Project</Text>}
                        ...
                        <List spacing={3} mt={8}>
                            {
                                project.content.map((text) => <ListItem key={text} color={colorSecondary[colorMode]}>
                                    <ListIcon as={ChevronRightIcon} color='primary' />
                                    {text}
                                </ListItem>)
                            }
                        ...
                        <Box mt={4}>
                            {
                                project.tags.map(tag => <Tag colorScheme='teal' key={tag} mt={4} mr={4}>{tag}</Tag>)
                            }
                        </Box>
                        ...
                        <Flex alignItems='center' justifyContent='space-between' width='80px' my={6}>
                            <Link href={project.githubURI} isExternal>
                                <Center>
                                    <Icon as={FaGithub} mx='2px' w={6} h={6} color={colorSecondary[colorMode]} _hover={{ color: '#64ffda' }} />
                                </Center>
                            </Link>
                            <Link href={project.liveURI} isExternal>
                                <Center>
                                    <ExternalLinkIcon mx='2px' w={6} h={6} color={colorSecondary[colorMode]} _hover={{ color: '#64ffda' }} />
                                </Center>
                            </Link>
                        </Flex>
        ...
        <Center w='100%'>
            <Button variant='outline' color='primary' onClick={handleViewMore}>View More</Button>
        </Center>
